...
void getAnswer(Graph* g, int src) {
    vector<int> oneHop;
    for (int idx = 0; idx < g->adj_edge[src].size(); idx++) {
        if (g->is_delete_e[g->adj_edge[src][idx]]) {
            continue;
        }
        int v = src;
        v ^= g->all_edge_pairs[g->adj_edge[src][idx]].v1;
        v ^= g->all_edge_pairs[g->adj_edge[src][idx]].v2;
        oneHop.push_back(v);
    }   
    queue<vector<int>> allScc;
    vector<int> tmpOneHop;
    tmpOneHop = oneHop;
    std::sort(tmpOneHop.begin(), tmpOneHop.end());
    for (auto v : oneHop) {
        if (!binary_search(tmpOneHop.begin(), tmpOneHop.end(), v)) {
            continue;
        }
        vector<int> needDelete;
        vector<int> currScc;
		currScc.push_back(src);
        currScc.push_back(v);
        auto position = find(tmpOneHop.begin(), tmpOneHop.end(), v);
        if (position != tmpOneHop.end()) {
            tmpOneHop.erase(position);
        }
        for (auto v1 : tmpOneHop) {
            if (g->adj_edge[v][v1]) {
                currScc.push_back(v1);
                needDelete.push_back(v1);
            }
        }
        allScc.push(currScc);
        for (auto v1 : needDelete) {
            position = find(tmpOneHop.begin(), tmpOneHop.end(), v1);
            if (position != tmpOneHop.end()) {
                tmpOneHop.erase(position);
            }
        }
	...
	vector<vector<int>> candicates;
	vector<vector<int>> existsNegative;
	vector<vector<int>> needExtend;
    int count = 1;
	int diameter = 0;
    while (!allScc.empty()) {
		diameter++;
        cout << "curr diameter is " << count << "\n";
		cout << "scc " << count++ << " is: \n";
        int size = allScc.size();
		for (int i = 0; i < size; i++) {

			vector<int> scc = allScc.front();
			allScc.pop();
			for (auto v : scc) {
				cout << v << " ";
			}
			cout << "\n";
			// TODO
			// check each scc. If not satisfy K-truss || exist unbalance  ——> need update
			// if satisfy K-truss and no unbalance 	——> mark as candicate
			// if exists candicate, choose the one with largest size, that is the answer
			// else 
			if (!getKruss()) {
				//needExtend.push_back(scc);
				allScc.push(...)
			} else {
				if (exists Negative Triangle) {
					existsNegative.push_back(scc);
				} else {
					candicates.push_back(scc);
				}
			}


			for (auto curr : existsNegative) {
				if (quickremoveNegativeTriangle(Graph with curr)) {
					if (still satisfy Ktruss) {
						candicates.push_back(curr);
					} else {
						allScc.push(curr);
					}
				}
...
int main(int argc, char** argv)  {
	...
	Graph *g = new Graph();
    getAnswer(original, start_vertex);
