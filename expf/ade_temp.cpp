...
void print_result(Graph* g){
	...
	cout << "final size of KTruss: " << final_size << endl;
...
vector<int> quick_delete(Graph* g, Triangle unb) {
	...
	if (hop_num[all_edge_pairs[unb.edge2].v1] == hop_num[all_edge_pairs[unb.edge2].v2]) {
	...
	if (hop_num[all_edge_pairs[unb.edge3].v1] == hop_num[all_edge_pairs[unb.edge3].v2]) {
	...
	while (!Queue.empty()) {
		...
		for (auto unb : Triangles) {
			...
			if (!first->Triangle_balance[unb.id] && !first->Triangle_broken[unb.id]){
				vector<int> quic_set = quick_delete(first, unb);			
				// if (quic_set.size() > 0) {
					// cout<<quic_set.size()<<endl;
				for (auto check : quic_set) {
					Graph *left = new Graph();
					*left = *first;

					if (delete_on_edge(check, left,false) && delete_on_radius(left) && left->size_of_truss > g->size_of_truss) {
				...
				// 	Graph *max_graph;
				// 	// cout<<"4\n";
				// 	if (delete_on_edge(unb.edge1, left,false) && delete_on_radius(left) && left->size_of_truss > g->size_of_truss) {
				// 		// bool best = false;
				// 		// cout<<"4.1\n";
				// 		// cout<< "checkkkk11\n";
				...
				// 	if (delete_on_edge(unb.edge2, middle,false)&&delete_on_radius(middle) && middle->size_of_truss >g->size_of_truss) {
				// 		// bool best = false;
				...
				// 	if (delete_on_edge(unb.edge3, right,false)&&delete_on_radius(right) && right->size_of_truss >g->size_of_truss)  {
				// 		// bool best = false;
...
int cc_edge_count(Graph* g, int v1) {
    vector<bool> visited(MAX_V, false);
    int edge_count = 0;
    queue<int> q;

    visited[v1] = true; // Mark v1 as visited
    q.push(v1); // Add v1 to the queue

    while (!q.empty()) { // While queue is not empty
        int v = q.front(); // Get the first element of the queue
        q.pop(); // Remove the first element from the queue

        // Traverse all neighboring vertices of the current vertex
        for (int i = 0; i < adj_edge[v].size(); i++) {
            int edge_id = adj_edge[v][i];  // Get the ID of the neighboring edge
            int v2 = all_edge_pairs[edge_id].v1;
            
            if (v2 == v) v2 = all_edge_pairs[edge_id].v2;

            if (!visited[v2] && !g->is_delete_e[edge_id]) {  // If the neighboring vertex hasn't been visited and the connecting edge hasn't been deleted
                edge_count++; // Increase edge count
                visited[v2] = true; // Mark neighboring vertex as visited
                q.push(v2); // Add neighboring vertex to the queue
            }
        }
    }
